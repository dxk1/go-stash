...
//		-appid
//			-funcid
//				-status_code rate(ret_code rate)
...
func RedisStory(info *DataInfo, timestamp float64) {
	if info == nil {
		return
	}
	...
	key := fmt.Sprintf("%s:%d", info.Region, timestamp)
	file := fmt.Sprintf("%s:%s:%s", info.Region, info.Cluster)
	value, _ := json.Marshal(info)
	redisClient.HSet(context.Background(), key, file, string(value))
	redisClient.SAdd(context.Background(), info.Region, key)
}
